"""depth_duration 更新四 — W2 配方 + RED 禁区（env VARIANT: X1=0.5%/30d, X1t=0.3%/30d）。

对照 W2（volume/update4_regime/runs/W2，+185.8%）。唯一差异：entry_signal 拒绝 RED 事件。

W1 = MA200 + COMBO优先 + 阀门θ=0.5
W2 = MA200 + COMBO优先 · 无阀
W3 = MA200 + 最近优先 · 无阀（隔离 COMBO：与 W2 仅差优先级）
其余规则同更新三（新池三条件、破线+4ATR 平仓、10坑×10万、15bps）。
...
THETA = {"W1": 0.5, "W2": None, "W3": None, "X1": None, "X1t": None}[VARIANT]

_ev = pd.read_parquet(HERE.parents[1] / "volume" / "update3_combo_backtest" / "events_combo.parquet")
...
# ── RED 禁区查表（贴顶∧顶新，analysis_014/depth_duration 更新三定稿）──
_rf = pd.read_parquet(HERE / "red_flags.parquet")
_rf["date"] = pd.to_datetime(_rf["date"])
_col = "REDt" if VARIANT == "X1t" else "RED"
REDSET = set(_rf.loc[_rf[_col], ["symbol", "date"]].itertuples(index=False, name=None))
print(f"[{VARIANT}] RED 禁区 {len(REDSET)} 日次（{_col}）", flush=True)
...
# ── MA200 硬开关（信号日沪深300 close > MA200，≤t 无未来）──
_con2 = duckdb.connect(str(WAREHOUSE), read_only=True)
_idx = _con2.execute("SELECT date, close FROM index_bar1d WHERE instrument='000300.SH' "
...
def select_order(open_pool: dict) -> list:
    if VARIANT == "W3":                               # 最近优先（对照：与 W2 仅差优先级）
