"""更新六策略 — W2 配方 + COMBO v2（严格按日标定，只用 date<当日 事件）。

与 update4_regime/W2 唯一差异：事件表 events_combo_v2.parquet（消除行序依赖）。
...
THETA = {"W1": 0.5, "W2": None, "W3": None}[VARIANT]

_ev = pd.read_parquet(HERE / "events_combo_v2.parquet")
