"""[ab_tests/A·C] degree≤5 过滤 + 最近突破优先（n 由引擎参数控制：A=10/100万，C=15/150万）。
对照 B=原版（degree≥1/n=10/最近优先，年化+7.84%）。分解更新二三改动的各自贡献。
...
def entry_payload(hist_df: pd.DataFrame) -> dict:
    ...
    修 2026-09-02（与 backtest_strategy/strategy.py v2 同步）：原 exit_check 以
    "入场日前最后一根 bar"重查 EVENT_MAP——池中排队 >1 天开仓的仓查不到信号日事件
    → line=None 丢止损线（只剩吊灯）。引擎把本载荷存入 holdings["signal_info"]。
    """
    if len(hist_df) == 0:
        return {}
    sym = hist_df["symbol"].iloc[-1]
...
def exit_check(entry_info: dict, hist_df: pd.DataFrame) -> bool:
    ...
    signal_info["line"]（entry_payload 入池载荷）；兜底：入场日前最后一根 bar 重查。
