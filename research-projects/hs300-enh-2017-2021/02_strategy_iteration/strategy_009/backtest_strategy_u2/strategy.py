...
更新二：偏离 [0%,+10%) 收紧（下界回到线上、上界 12%→10%）+ 引擎 30×10万（300万）——
        去掉更新一允许的贴线下段(-2~0)，只做"线上 0~10%"半防守，测能否兼得躲熊与吃牛
...
ATR_MULT = 4.0
MAX_DEGREE = 5
DEV_LO, DEV_HI = 0.00, 0.10       # 指数偏离年线 [0%,+10%) 闸门（更新二收紧）
POOL_TIMEOUT_DAYS = 5
POOL_DD = 0.10
...
# ── 指数偏离 MA200 ∈[DEV_LO,DEV_HI) 日期集合（≤t，无未来）──
...
def entry_signal(hist_df: pd.DataFrame) -> bool:
    """开仓：v4 突破(degree≤5) + 当日成分 + 指数偏离∈[DEV_LO,DEV_HI) 闸门。"""
    if len(hist_df) == 0:
        return False
