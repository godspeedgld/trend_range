# industry _regime_stock_picking — 综合报告

> 方向：行业 regime 选股（行业信息在选股框架中的用法：方向 vs 过滤）。

## 引用研报

| 卡 | 引用方 |
|---|---|
| `research_report/rotation_regime_index_yinhe_20260701_main.md` | analysis_001（变盘指数六轮） |
| `research_report/industry_research/property_dip_accumulation_caitong_20260130_main.md` | （背景，暂无 analysis 引用） |
| `research_report/industry_research/property_reversal_targets_caitong_20260531_main.md` | **analysis_002（PB 回归法验证）** |
| `research_report/industry_research/property_hk_mapping_timing_caitong_20260625_main.md` | **analysis_002（0625 预测回验节）** |
| `research_report/industry_research/property_tier2_stabilization_caitong_20260915_main.md` | （背景，暂无 analysis 引用） |

## 分析与策略索引

- analysis_001：银河「变盘指数」六轮全负（见其 records.md）
- **analysis_002**：验证财通 PB 回归法 —— 六家 A 股房企 PB 分位/十分位/修复目标价
  （关键发现：BigQuant pb 口径 = 全权益非归母，已修正；0625 温和上行预测 +7.4% 兑现）
