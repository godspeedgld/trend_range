...
| **quantreg_sr_algo_v2.py** | 动态窗口分位回归线（LOO 离群截断）| 锁定（09-09）| v1 固定 120 窗 → v2：因果极值后对每侧转折点 LOO 检测(>余点 mean+2σ)，截断边界=最新离群点，**只向过去截**(截后<N_min60 回退全窗防误截)，≤2 轮，每侧≥3 点即可拟合。窗含 regime 急切换时不被单一离群点钉住（v1 曾一个点致斜率翻号 724%）|

> v1/v2 为分位回归线（Lang2012/渤海路线）两个版本；**与 plateau 家族为不同范式**。绩效经验（analysis_013 更新九）：该线家族与"波动出场+窄偏离带"脚手架配套（band06 +42%），塞进 plateau 的破线止损+MA200 脚手架会塌到 +7%——勿跨脚手架混用。
