...
[建模，含 flow_d]
### 2.1 模型与最优阶数
- 流程：`<flow_a/b/c/d>`（均值方程 `<…>` + 方差方程 `<…>`）
- 模型类型 / 最优阶数（AIC/BIC；ARFIMA 含 d；flow_d 为 ((0,0),(0,0))）/ 样本量 / AIC / BIC

### 2.2 参数估计
<参数名 / 参数值 表>（flow_d = {mu, sigma2}；GJR 含 vol_gamma[1] 杠杆系数）

### 2.3 建模后 Ljung-Box 检测
- flow_a / flow_d：均值方程残差（创新）Ljung-Box 表
...
- **flow_d**（均值=Constant 且 方差=Constant）：拟合常数模型 μ、σ²，出参数、残差 LB 与预测图（μ 的水平线）。
