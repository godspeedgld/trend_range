...
落到四流程之一（均值×方差 9 种组合全覆盖），按 AIC/BIC 选阶、估计参数，再用 Ljung-Box 验证残差，
...
|---|---|---|---|
| **Constant**（无自相关、无长记忆） | `flow_d` 常数模型(μ, σ²) | `flow_b` GARCH | `flow_b` GJR-GARCH |
...
- 均值 3 × 方差 3 = 9 格，**9 种建模组合全覆盖**（常数均值+不变方差也拟合为 `flow_d` 基线模型）。
...
收益率序列（必须平稳）
   ...
   │
   ├── flow_d  ─► 常数均值 + 不变方差 → 拟合 μ、σ² → 残差 LB → 判定
...
- `fit_model(diag, series, *, max_p=3, max_q=3, p_max=2, q_max=2, criterion="aic", forecast_steps=20)` — 按 `diag.flow` 路由 flow_a/b/c/d（9 种组合全覆盖）
- `flow_d(series, ...)` / `flow_a(series, mean_eq, ...)` / `flow_b(series, var_eq, ...)` / `flow_c(series, mean_eq, var_eq, ...)` — 四流程
