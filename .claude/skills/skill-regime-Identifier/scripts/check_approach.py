#!/usr/bin/env python3
"""Step 3 门禁 — 方法提取检查（3类 taxonomy，不含止盈止损/风控）"""
import sys, os, re, io
...
def check(project_dir):
    ...
    print("=" * 70); print("Step 3 门禁 — 方法提取（main_approach.md）"); print("=" * 70)
    if not doc_path.exists():
        errors.append("[FAIL] 02_approach/main_approach.md 不存在")
        _summary(errors, warnings); return 1
    size = doc_path.stat().st_size
    if size < 1500:
        errors.append(f"[FAIL] main_approach.md 过短 ({size} bytes)")
    else:
    ...
    checks = [
        ...
        ("优缺点", [r"优点", r"缺点", r"延迟", r"抗噪", r"敏感", r"不足", r"滞后"]),
        ("衍生方法", [r"衍生", r"替代", r"未采用", r"提及"]),
