...
2. **本地引擎参数语义冻结**：移植前逐个确认本地回测引擎参数的真实语义再翻译
   ...
   ⚠ **止损/吊灯的价格基准"起算点"必须单独确认**（池信号日 / 决策日 / 入场日 三者不同）。
   正确语义 = **开仓后最高点**（用户本意，见 W2 确认 2026-09-11）；本地 `exit_check` 用
   `sig_idx = 入场日前一交易日`（决策日）= 该语义；而**池内失效**条件用**池信号日**起算——
   **同一策略里两处口径本就不同，必须分别对齐**。
   若把持仓吊灯错锚在池信号日（池无超时/长排队时信号→入场可隔数十日），会把建仓前的高点
   算进吊灯 → 系统性提前出场。**实证**（W2 移植）：600036 2017-12-13 入场，
   池信号日(11-17)起 peak 146.55→floor 130.65 vs 决策日(12-12)起 peak 134.56→floor 118.66，
   12-15 close 129.88 → 前者触发（12-18 卖 −1.03%）、后者不触发（持到次年 3 月 +5.93%）。
   规则：**持仓吊灯 = 开仓（决策日）起算；池失效 = 池信号日起算**，逐字节对齐。
...
**移植到平台时，除算法代码外，不得上传任何本地中间产物**（事件表/指标表/信号表/因子值/
已标定的参数表等）。平台侧必须是 **算法代码 + 原始数据** 的完整重建。

- ❌ 禁止：把本地 `events.parquet` / `signals.csv` / `features.parquet` / 预计算好的 COMBO 等
  当成输入上传，让云端"查表跑回测"——那样验证的是查表，**不是算法可移植性**，且本地任何
  口径/前视/污染都会原样带进云端且无法察觉
- ✅ 正确：本地的每一层预计算（事件状态机、指标如 ATR、量能特征、打分标定…）都在云端用
  **独立 cell 重新计算**，计算完**保存成文件**（`cloud_export/` 或工作区文件）——
  保存的目的不是喂给回测，而是**出问题时能下载下来与本地逐值对账**
- 判据：云端 notebook 从"原始数据表"出发能独立走完 数据 → 指标 → 事件 → 信号 → 回测 全链；
  把所有本地中间产物删掉，云端仍应从 raw data 复现出同样结果
- 唯一例外：**算法代码本身**（状态机/指标定义/打分逻辑）必须移植——这正是被验证的对象

> 与「数据获取/处理」步骤的关系：预计算不是不做，而是**在云端 cell 里做**，并且落盘可查。
> 该铁律与「严禁未来数据」同级——违反其一，移植验收结论作废。
