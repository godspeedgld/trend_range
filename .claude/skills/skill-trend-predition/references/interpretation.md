...
- **低延迟变体 `adx_hma`**：把 DX→ADX 的 Wilder SMMA 换成 HMA（DI 仍标准 Wilder）。优点是滞后更低、趋势反转更早（HC 日线实测领先标准 ADX 约 5 根）；代价是对噪声更敏感、warmup 略长。当规则要"更早确认趋势切换"时用它；要"与教科书/talib 对齐"用标准 `adx`。
