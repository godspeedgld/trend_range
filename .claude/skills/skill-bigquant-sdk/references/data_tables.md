...
**额度估算（沪深300 成分股 · 2020-01-01 至今，2026-09-15 测算）**

前提：**1,624 个交易日**（2020-01-02 ~ 2026-09-11，本地 `trading_days` 实测）
× **240 bar/日**（A股 9:30-11:30 + 13:00-15:00；本地无 1m 数据可实测，按标准取值）× **9 列**。

| 口径 | 行数 | 单元格 | 周配额 |
|---|---|---|---|
| **严格成分股-日**（每日仅实际 300 只） | 1.17 亿 | 10.5 亿 | **≈1052%（≈10.5 周）** |
| 并集面板（668 只 × 全期，日均 ~600 只） | 2.34 亿 | 21.0 亿 | **≈2105%（≈21 周）** |
| 参考：近 1 年（243 日）× 严格 300 只 | 1,750 万 | 1.57 亿 | ≈157%（≈1.6 周） |

- 基准：**周配额 = 1 亿单元格**
- ⚠ **这是本仓库最贵的表**——全期沪深300 分钟数据 ≈ **10.5 周配额**，一次拉不完，必须按月分区断点续传
  （该表云端本来就是**按月截面存储**，天然适合 `filters={"date": [月初, 月末]}` 逐月拉）
- 缩量三招：① 只拉需要的股票子集（如仅出过突破信号的股票）② 只拉关键时段（信号日前后 N 天）
  ③ 按月分批，配额滚动释放后继续
- 并集口径比严格口径**贵一倍**（~600/日 vs 300/日），此处不建议用并集（与日线表不同，分钟表量级太大）
...
**★ 实用子集——8 个「最终因子」**（研究直接用这 8 列；中间因子只是构造过程的分步产出，一般不需要）：

| 字段 | 名称 | 字段 | 名称 |
|---|---|---|---|
| alpha_91001 | 花隐林间 | alpha_91022 | 球队硬币 |
| alpha_91007 | 适度冒险 | alpha_91027 | 飞蛾扑火 |
| alpha_91012 | 云开雾散 | alpha_91041 | 多空博弈 |
| alpha_91018 | 勇攀高峰 | alpha_91019 | 灾后重建 |

⚠ 拉取时显式 SELECT 需用的 alpha 列（此表列较多，实际含 5x+ 因子列），禁 `SELECT *`——先查
本表所需因子清单再拉（见 SKILL.md 列拉取铁律）。
**只拉这 8 个最终因子 = 10 列**（date + instrument + 8 alpha），是性价比最高的口径：
沪深300 历史成分股 2015 至今约 **1,700 万单元格 ≈ 17% 周配额**（详见下表后的额度估算）。
...
**额度估算（沪深300 成分股 · 2015-01-01 至今，2026-09-15 实测）**

| 口径 | 行数 | 列数 | 单元格 | 周配额 |
|---|---|---|---|---|
| **只拉 8 个最终因子** × 并集面板（668 只 × 2828 交易日） | 1,698,600 | 10 | 1,699 万 | **≈17.0%** |
| 只拉 8 个最终因子 × 严格成分股-日（每日仅实际 300 只） | ~848,000 | 10 | 848 万 | **≈8.5%** |
| 全列 × 并集面板（date + instrument + 44 个 alpha，❌ 反面教材） | 1,698,600 | 46 | 7,814 万 | **≈78%** |

- 基准：**周配额 = 1 亿单元格**（`146万行×13列≈1900万=19%`，见 `fetch_industry_bar1d.py` docstring）
- 668 只 = 沪深300 **历史成分股全集**（2015-2026 曾入选者）；本地 `stock_bar1d` 是**并集面板**
  （668 只 × 全交易日，日均 ~600 行），**非**每日实际 300 只
- 严格成分股-日需 join `cn_stock_index_component` 逐日过滤；并集面板口径更省事，
  多出的"非成分日"在后续 join 分析时会自然丢掉
