...
**策略型 时序 CTA / 趋势跟踪研报复现技能**。把一篇含**可交易策略**的时序 CTA 研报、论文、PDF、网页或文本（或方法论综述的实证策略部分），端到端转化为完整复现交付包：

**全文中文翻译 → 策略逻辑抽取（regime / 开仓 / 止盈止损 / 仓位控制 / 参数优化）→ 回测策略代码 → 真实本地回测 → 中文评估报告 → 交付摘要**

## 适用 / 不适用

- ✅ **适用**：策略型时序 CTA 论文（含具体开仓/退出规则），或方法论综述的实证策略部分。
- ❌ **不适用**：纯理论/regime 综述（无可交易策略）、截面因子研报（用 `skill-report-replication-factor`）。

## 抽取默认值（研报未明确时）

策略型研报侧重各异（开仓 / 止盈止损 / regime / 仓位 / 参数优化 / 综合）。研报未给的要素填默认值：
